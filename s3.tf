...
resource "aws_s3_bucket" "apiary_data_bucket" {
  ...
  tags          = "${merge(map("Name", "${element(local.apiary_data_buckets, count.index)}"), map("Product", "${element(local.apiary_s3_bucket_tags_product, count.index)}"), map("Domain", "${element(local.apiary_s3_bucket_tags_domain, count.index)}"), "${var.apiary_tags}")}"
